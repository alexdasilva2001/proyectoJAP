...
document.addEventListener('DOMContentLoaded', () => {
  // Buscamos por id o por clase como fallback
  ...
  // Si falta algo, avisamos y no rompemos la ejecución
  if (!editarBtn) {
    console.error('my-profile.js: no se encontró el botón de editar foto (id="editarFotoBtn" o class="editar-foto")');
    return;
  }
  if (!inputFoto) {
    console.error('my-profile.js: no se encontró el input file (id="inputFoto"). Añadilo al HTML.');
    return;
  }
  if (!fotoPerfil) {
    console.error('my-profile.js: no se encontró la imagen de perfil (id="fotoPerfil" o class="foto-perfil")');
    return;
  }
  ...
  inputFoto.addEventListener('change', (e) => {
    ...
    reader.addEventListener('load', (ev) => {
      ...
      // Intentamos guardar en localStorage (opcional)
      try {
  ...
  try {
    ...
    // localStorage puede estar deshabilitado por privacy mode
  }
});
